  ...
  **************************************************/
  ...
  static void ntt_stage(const int16_t in[256], int16_t out[256]) {
    ...
    for (int i = 0; i < 128; i++) {
      ...
      // builds 16 butterflies working in parallel
  ...
  **************************************************/
  ...
  void ntt(const int16_t in[256], int16_t out[256]) {
    ...
    #pragma HLS ARRAY_PARTITION variable=buf1 cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=buf2 cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=buf3 cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=buf4 cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=buf5 cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=buf6 cyclic factor=32 dim=1
    // dataflow style
  ...
  **************************************************/
  ...
  void hls_poly_basemul(int16_t r[256], const int16_t a[256], const int16_t b[256]) {
    #pragma HLS INLINE off
    for (int i = 0; i < 64; i++) {
      // unroll 8 touches indices 32i..32i+31 per cycle: one element per bank, given
      // the caller's buffers are cyclic-32 partitioned
      #pragma HLS PIPELINE II=1
  ...
  **************************************************/
  ...
  static void copy_poly(const int16_t in[256], int16_t out[256]) {
    #pragma HLS INLINE off
    for (int i = 0; i < 256; i++) {
      // 32 coefficients per cycle: needs the caller's buffers cyclic-32 partitioned
      #pragma HLS PIPELINE II=1
  ...
  **************************************************/
  void poly_mul(const int16_t a[256], const int16_t b[256], int16_t r[256]) {
    ...
    #pragma HLS ARRAY_PARTITION variable=ta cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=tb cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=na cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=nb cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=tr cyclic factor=32 dim=1
    #pragma HLS ARRAY_PARTITION variable=ti cyclic factor=32 dim=1
